...
const { getAuthenticatedClient, logout, authenticateUser } = require('./auth');
...
async function handleCallback(bot, callbackQuery) {
  ...
  try {
    // Handle client type selection (no auth required)
    if (data === 'client_type_new') {
      await handleNewClientFlow(bot, chatId, messageId, userId);
      await bot.answerCallbackQuery(callbackQuery.id);
      return;
    }
    
    if (data === 'client_type_existing') {
      await handleExistingClientFlow(bot, chatId, messageId);
      await bot.answerCallbackQuery(callbackQuery.id);
      return;
    }
    
    // Handle intake form callbacks (no auth required)
    if (data.startsWith('intake_')) {
      await handleIntakeCallback(bot, chatId, messageId, userId, data);
      await bot.answerCallbackQuery(callbackQuery.id);
      return;
    }
    
    // Handle quick auth callback
    if (data.startsWith('quick_auth_')) {
      const authCode = data.replace('quick_auth_', '');
      await handleQuickAuth(bot, chatId, messageId, userId, authCode);
      await bot.answerCallbackQuery(callbackQuery.id);
      return;
    }
    
    // Check authentication for all other actions except logout
    if (data !== 'logout') {
...
async function downloadClientFile(bot, chatId, client, fileId) {
  try {
    ...
    if (fileData) {
      // Send the file directly (data is already a Buffer)
      await bot.sendDocument(chatId, fileData.data, {
        filename: fileData.name,
...
async function handleNewClientFlow(bot, chatId, messageId, userId) {
  // Check if intake session already exists, if not create one
  let session = getIntakeSession(userId);
  if (!session) {
    session = startIntake(userId);
  }
  
  const content = getStepContent(session.step, session.data);
  
  try {
    const sentMessage = await bot.editMessageText(content.message, {
      chat_id: chatId,
      message_id: messageId,
      parse_mode: 'Markdown',
    ...
    session.messageId = messageId; // Keep the original message ID
  } catch (error) {
    // If edit fails, send new message
    const sentMessage = await bot.sendMessage(chatId, content.message, {
      parse_mode: 'Markdown',
      reply_markup: content.keyboard ? content.keyboard.reply_markup : undefined
    });
...
async function handleExistingClientFlow(bot, chatId, messageId) {
  const existingClientMessage = `
🔑 **Existing Client Authentication**

Please authenticate with your unique auth code:

**Example:** \`/auth ABC123XYZ\`

🔑 You should have received your auth code from us. If you don't have one, please contact support.
...
• \`/help\` - Show help message
  ...
  try {
    await bot.editMessageText(existingClientMessage, {
      chat_id: chatId,
      message_id: messageId,
      parse_mode: 'Markdown'
    });
  } catch (error) {
    await bot.sendMessage(chatId, existingClientMessage, {
      parse_mode: 'Markdown'
    });
  }
}

async function handleIntakeCallback(bot, chatId, messageId, userId, data) {
  const session = getIntakeSession(userId);
  
  if (!session) {
    await bot.sendMessage(chatId, 'Session expired. Please start over with /start');
    return;
  }
  
  const result = processCallback(session, data);
  
  if (result.success) {
    if (result.submit) {
      // Submit the form
      await submitIntakeForm(bot, chatId, messageId, userId, session);
      return;
    }
    
    if (result.cancel) {
      // Cancel the form
      clearIntakeSession(userId);
      await bot.editMessageText(
    ...
    // Continue to next step or refresh current step
    const content = getStepContent(session.step, session.data);
    
    try {
      await bot.editMessageText(content.message, {
        chat_id: chatId,
        message_id: session.messageId || messageId,
        parse_mode: 'Markdown',
        reply_markup: content.keyboard ? content.keyboard.reply_markup : undefined
      });
    } catch (error) {
      ...
      // Only send new message if it's a critical error (message not found, etc.)
      if (error.code === 'ETELEGRAM' && error.response?.body?.error_code === 400) {
        try {
          ...
          // Update session with new message ID for future edits
          session.messageId = sentMessage.message_id;
        } catch (sendError) {
          console.error('Error sending new message:', sendError);
        }
      }
      // For other errors, just log and continue - the message might still be visible
    }
  } else {
    // Show error message
...
async function submitIntakeForm(bot, chatId, messageId, userId, session) {
  try {
    // Generate auth code
    ...
    const result = await addNewClient(clientData);
    
    if (result.success) {
      // Clear session
      clearIntakeSession(userId);
      
      // Send success message with auth code
      const successMessage = `
🎉 **Welcome to EVO Labs!**

Your intake form has been submitted successfully! 

**Your unique auth code:** \`${authCode}\`
...
**Next Steps:**
1. Save your auth code: \`${authCode}\`
2. Use \`/auth ${authCode}\` to access your dashboard
...
🎯 **Project Type(s):** ${clientData.projectTypes.join(', ')}
⏰ **Timeframe:** ${clientData.timeframe}
💰 **Budget:** ${clientData.budget}

🚀 **Ready to get started?** Use \`/auth ${authCode}\` now!
      `;
      ...
      // Send follow-up message with quick auth button
      const authKeyboard = {
        reply_markup: {
          inline_keyboard: [
            [{ text: `🔐 Authenticate with ${authCode}`, callback_data: `quick_auth_${authCode}` }]
          ]
        }
      };
      
      await bot.sendMessage(chatId, 
        '🔑 **Quick Access:** Tap the button below to authenticate instantly!',
        {
          parse_mode: 'Markdown',
          ...authKeyboard
        }
...
async function handleQuickAuth(bot, chatId, messageId, userId, authCode) {
  try {
    const result = await authenticateUser(userId, authCode);
    
    if (result.success) {
      const client = getAuthenticatedClient(userId);
      
      await bot.editMessageText(
        `✅ **Successfully authenticated!**\n\nWelcome ${client.clientName}! 🎉\n\n🔔 Notifications are now enabled for job updates.`,
        {
          chat_id: chatId,
          message_id: messageId,
          parse_mode: 'Markdown'
      ...
      // Show main menu
      const { createMainMenu } = require('./keyboards');
      const keyboard = createMainMenu(client.clientName);
