    ...
    if (data !== 'logout') {
      ...
      switch (true) {
        ...
        case data.startsWith('jobfiles_'):
          const [category, jobIdFromFiles] = data.replace('jobfiles_', '').split('_');
...
async function showJobFiles(bot, chatId, messageId, client, jobId) {
  try {
    // Get files from client's Google Drive folder using their client code
    ...
    if (!files || files.length === 0) {
      ...
      await bot.editMessageText(
        `📎 **Files for Job ${jobId}:**\n\n📁 No files found in your client folder.\n\nFiles should be located in your dedicated Google Drive folder.`,
        {
          chat_id: chatId,
          message_id: messageId,
          parse_mode: 'Markdown',
    ...
    await bot.editMessageText(
      `📎 **Files for Job ${jobId}:**\n\n📁 Found ${files.length} file${files.length === 1 ? '' : 's'} in your client folder:`,
      {
        chat_id: chatId,
        message_id: messageId,
        parse_mode: 'Markdown',
...
async function showJobFileCategory(bot, chatId, messageId, client, category, jobId) {
  try {
    // Get files from client's Google Drive folder
    const files = await getClientFiles(client.clientCode);
