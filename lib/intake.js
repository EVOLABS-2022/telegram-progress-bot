...
function getStepContent(step, data = {}) {
  switch (step) {
    ...
    case FORM_STEPS.PROJECT_TYPE:
      ...
      const typeButtons = [
        { text: `${selectedTypes.includes('Web/Mobile Development') ? '✅ ' : '📱 '}Web/Mobile Development`, callback_data: 'intake_type_web' },
        { text: `${selectedTypes.includes('Web3 Development') ? '✅ ' : '🔗 '}Web3 Development`, callback_data: 'intake_type_web3' },
        { text: `${selectedTypes.includes('2D/3D Animation') ? '✅ ' : '🎬 '}2D/3D Animation`, callback_data: 'intake_type_animation' },
        { text: `${selectedTypes.includes('Digital Art/Graphics') ? '✅ ' : '🎨 '}Digital Art/Graphics`, callback_data: 'intake_type_art' },
        { text: `${selectedTypes.includes('Mixed Media') ? '✅ ' : '🎭 '}Mixed Media`, callback_data: 'intake_type_mixed' }
      ];
      
      return {
        message: `✅ Name: ${data.name}\n✅ Email: ${data.email}\n${data.company ? `✅ Company: ${data.company}\n` : ''}${data.phone ? `✅ Phone: ${data.phone}\n` : ''}\n🎯 **Project Type** \\* (select all that apply)\n\n${selectedTypes.length > 0 ? `**Selected:** ${selectedTypes.join(', ')}\n\n` : ''}_Choose your project type(s):_`,
        keyboard: {
          reply_markup: {
            inline_keyboard: [
              ...typeButtons.map(btn => [btn]),
              selectedTypes.length > 0 ? [{ text: '✅ Continue', callback_data: 'intake_continue' }] : []
...
function processCallback(session, callbackData) {
  switch (true) {
    ...
    case callbackData.startsWith('intake_type_'):
      ...
      if (selectedType) {
        // Initialize projectTypes array if it doesn't exist
        if (!session.data.projectTypes) {
          session.data.projectTypes = [];
        }
