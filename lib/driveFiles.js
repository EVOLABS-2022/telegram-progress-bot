...
async function findClientFolder(clientCode) {
  ...
  try {
    ...
    if (!clientFilesSearch.data.files.length) {
      // Let's see what folders ARE in the shared drive root
      const rootFolders = await drive.files.list({
        q: `mimeType='application/vnd.google-apps.folder' and parents in '${SHARED_DRIVE_ID}'`,
        includeItemsFromAllDrives: true,
        supportsAllDrives: true,
        fields: 'files(id, name)'
      });
      console.log('All folders in shared drive root:', rootFolders.data.files.map(f => f.name));
